...
@Repository
public class ContenidoSearchPostgresRepo implements ContenidoSearchDAO{
	...
	public List<Long> getContenidosIDByPrompt(String prompt){
		...
		final String vector = "to_tsvector('spanish',coalesce(cast(\"ID\" as VARCHAR),'')||' '||coalesce(\"Titulo\",'')||' '||coalesce(\"Autor\",'')||' '||coalesce(\"Descripcion\",''))";
		final String sqlString = "SELECT \"ID\", ts_rank("+vector+", "+query+") AS \"RANK\" FROM \"Contenidos\" WHERE "+vector+" @@ "+query+" ORDER BY \"RANK\";";
	...
	public List<Long> getContenidosIDBySoporte(Soporte soporte) {
		...
		return jdbcTemplate.query(sqlString, new MapSqlParameterSource().addValue("soporte", "%"+soporte.toString()+"%"), Utils.idRowMapper);
	}
